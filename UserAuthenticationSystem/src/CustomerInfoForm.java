...
public class CustomerInfoForm extends JFrame {
	...
	private JButton signOffButton = new JButton("Sign Out");
	...
	public CustomerInfoForm(String username) {
		...
		hGroup.addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(userIDLabel)
				.addComponent(usernameLabel).addComponent(passwordLabel).addComponent(nameLabel)
				.addComponent(phoneLabel).addComponent(emailLabel).addComponent(addressLabel).addComponent(countryLabel)
				.addComponent(cityLabel).addComponent(zipLabel).addComponent(regionLabel).addComponent(editButton));
		hGroup.addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(userIDValueLabel)
				.addComponent(usernameField).addComponent(passwordField).addComponent(nameField)
				.addComponent(phoneField).addComponent(emailField).addComponent(addressField).addComponent(countryField)
				.addComponent(cityField).addComponent(zipField).addComponent(regionField).addComponent(saveButton)
				.addComponent(toggleButton));
		hGroup.addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(userIDValueLabel)
				.addComponent(usernameField).addComponent(passwordField).addComponent(nameField)
				.addComponent(phoneField).addComponent(emailField).addComponent(addressField).addComponent(countryField)
				.addComponent(cityField).addComponent(zipField).addComponent(regionField).addComponent(cancelButton)
				.addComponent(signOffButton));

		// Add components to vertical group
		vGroup.addGroup(layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(userIDLabel)
		...
		vGroup.addGroup(layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(toggleButton));
		...
		vGroup.addGroup(layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(editButton)
				.addComponent(saveButton).addComponent(cancelButton).addComponent(signOffButton));
		...
		// Intially hide the toggleButton so that user cannot show password without
		// editing
		...
		saveButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Hide show password and cancel edits buttons once user saves their edited
				// information
				cancelButton.setVisible(false);
				toggleButton.setVisible(false);
		...
		signOffButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				dispose(); // Closes the customerForm opened
	...
	private void fetchUserData(String username) {
		String query = "SELECT user_id, login.username, originalPasses.pass, name, phone, email, address, country, city, zip, region "
				+ "FROM customerData " + "JOIN login ON customerData.cust_reg_num = login.id "
				+ "JOIN originalPasses ON originalPasses.id = login.id " + "WHERE username = ?";
	...
	private void saveUserData(String username) {
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			String hashedPassword = UserAuthenticationSystem.hashPassword(passwordField.getText()); // Hash the password
																									// before udpating.
